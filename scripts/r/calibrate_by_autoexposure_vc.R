...
#args <- c(44.98965, -93.1854, 44.9894, -93.1865) #small field
args <- c(45, -93, 44, -94) #whole field
#setwd("/Users/ianmcnish/Google\ Drive\ File\ Stream/My\ Drive/projects/2017_btn/working_photos/btn_2017_06_12_flight_1_120ft")
...
foreach(i=1:length(channels)) %dopar%{
  ...
  gps_data <- read.csv(paste0(channels[i],"pix4d.csv"))
  ...
  #mean_Dn <- vector(length=length(file_paths), mode = 'numeric')
  ...
  #  mean_Dn[ii] <- mean(temp_photo)
  #  #assign(paste('photo',ii,sep=""),temp_photo)
  #}
  #correction <- 0.1/mean(mean_Dn)
  ...
  for (iii in 1:length(file_paths)){
    if(gps_data$Lat..decimal.degrees.[iii] < args[1] &
       gps_data$Lon..decimal.degrees.[iii] < args[2] &
       gps_data$Lat..decimal.degrees.[iii] > args[3] &
       gps_data$Lon..decimal.degrees.[iii] > args[4]){
    #this code correct the number of channels in single channel photos
    if (i==1){
