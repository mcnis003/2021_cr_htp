...
ground_control_point_info$approximate_latitude_min <- ground_control_point_info$approximate_latitude - 0.0002
ground_control_point_info$approximate_latitude_max <- ground_control_point_info$approximate_latitude +  0.0002
ground_control_point_info$approximate_longitude_min <- ground_control_point_info$approximate_longitude -  0.0002
ground_control_point_info$approximate_longitude_max <- ground_control_point_info$approximate_longitude +  0.0002

ground_control_point_directories <- ground_control_point_info$ground_control_point_name

for (i in 1:length(photo_directories)){
  
  photo_list <- list.files(path = calibrated_photos_directories[i], pattern = "*.jpg", full.names = TRUE)
  photo_info <- read.csv(paste0(photo_directories[i], "pix4d.csv"), header = TRUE)
  #photo_info$File.Name <- sub(pattern = "-", replacement = "_", x = photo_info$File.Name)
  ...
  cat(paste0("begin moving ", photo_directories[i], "\n"))
    ...
    for (ii in 1:length(ground_control_point_directories)){
      ...
      for (iii in 1:length(photo_info_temp$File.Name)){
        ...
        from_file <- paste0(calibrated_photos_directories[i], "calibrated_", photo_info_temp$File.Name[iii])
        to_file <- paste0(calibrated_photo_gcp_directories[i], ground_control_point_directories[ii], "/", "calibrated_", photo_info_temp$File.Name[iii])
