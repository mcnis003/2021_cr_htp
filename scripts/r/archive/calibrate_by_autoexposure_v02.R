...
foreach(i=1:length(channels)) %dopar%{
  ...
  sink(file=paste(args[1],correction_file_name[i],sep=""))
  cat(correction[i])
  sink()
  ...
  for (ii in 1:length(file_paths)){
    ...
    writeTIFF(correction[i]*photo,paste(args[1],folders[i],"/calibrated_",file_names_tiff[ii],sep=""), bits.per.sample = 16L)
    ...
    to_file=paste(args[1],folders[i],"/calibrated_",file_names_tiff[ii],sep="")
